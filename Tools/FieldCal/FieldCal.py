...
def proc_args():
    ...
    ideal_map_measurements = sorted(load_measurements(ideal_map_json), key=lambda x: x[0])
    calibrated_map_measurements = sorted(load_measurements(measured_map_json), key=lambda x: x[0])
