...
void GoToPositionCommand::Initialize() {
  ...
  if ((currentArmPosistion < 90_deg && m_position.arm_angle > 90_deg) || (currentArmPosistion > 90_deg && m_position.arm_angle < 90_deg)){
...
bool GoToPositionCommand::IsFinished() {
  if(m_pElevatorSubsystem->IsElevatorAtSetPoint() && m_pElevatorSubsystem->IsArmAtSetPoint() && m_pElevatorSubsystem->IsWristAtSetPoint()){
    return true;
  }
  return false;
}
